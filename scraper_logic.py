...
def get_preview(query, limit):
    """Mengambil banyak URL gambar dengan berbagai pola pola scraping"""
    headers = {
    ...
    # Menambahkan rs=typed dan memformat query agar hasil lebih luas
    ...
    try:
        ...
        # Mencari semua pola URL gambar (Original, 736x, dan 474x)
        # Pinterest menyimpan link gambar di dalam script JSON di HTML
        patterns = [
        ...
        # Menghapus duplikat sambil menjaga urutan
        unique_urls = []
        for url in found_urls:
            if url not in unique_urls:
                unique_urls.append(url)

        # Jika hasil nol, coba pola yang lebih umum
        if not unique_urls:
...
def download_and_zip(urls, query):
    """Proses unduh dan bungkus ke ZIP"""
